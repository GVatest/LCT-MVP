...
export const signup = ({username, fullName, password, password2, is_superuser}) => async dispatch => {
    const config = {
        headers: {
            "Content-Type": "application/json",
        }
    ...
    const body = JSON.stringify({username, fullName, password, password2, is_superuser})

    try {
        const res = await axios.post(BASE_URL + "api/user/signup/", body, config)
